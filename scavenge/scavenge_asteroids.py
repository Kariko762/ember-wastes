# scavenge/scavenge_asteroids.py
import json
import os
import random
...
def scavenge(player_data, location, ship_inventory, energy_cost, scavenge_data):
    ...
    items = scavenge_data["locationMappings"][loc_type]["resources"]
    ...
    if roll > success_chance:
        print(f"Scanning Asteroid Field {roll_str} ({roll}%): Success (Threshold {success_chance}%)")
        ...
        print(f"Scanning Asteroid Field {roll_str} ({roll}%): No Resources Found (Threshold {success_chance}%)")
